SMODS.Consumable {
    ...
    loc_txt = {
        name = 'Templ-8',
        text = {
        [1] = 'Level up a {C:planet}random hand{} by {C:attention}2{}',
        [2] = '{C:green}1 in 4{} Chance to also Level up',
        [3] = 'another hand by {C:attention}4{}'
    }
    },
    cost = 3,
    unlocked = true,
    ...
    atlas = 'CustomConsumables',use = function(self, card, area, copier)
        local used_card = copier or card
            ...
            delay(1.3)
            if SMODS.pseudorandom_probability(card, 'group_0_cdc44989', 1, card.ability.extra.odds, 'c_inkysupg_templ8', false) then
            ...
            update_hand_text({ sound = 'button', volume = 0.7, pitch = 0.9, delay = 0 }, { level = '+'..tostring(4) })
            delay(1.3)            
            local hand_pool = {}
            for hand_key, _ in pairs(G.GAME.hands) do
                table.insert(hand_pool, hand_key)
            ...
            local random_hand = pseudorandom_element(hand_pool, 'random_hand_levelup')
            level_up_hand(card, random_hand, true, 4)
